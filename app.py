...
def obter_data_atualizacao():
    """
    Retorna a data e hora da última execução do script de atualização,
    registrada na tabela log_atualizacoes.
    """
    try:
        DB_URL = get_db_url()
        if not DB_URL:
        ...
        # Buscar a data da última execução do script (QUALQUER execução)
        cur.execute("""
        ...
        if resultado:
            return resultado[0]  # Retorna a data da última execução
        else:
            # Se não houver log, usa a data atual como fallback
            return datetime.now()
